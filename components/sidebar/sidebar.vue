<template>
	<client-only>
		<div
			class="bg-white md:relative md:w-[260px] z-10 transition-all duration-300 fixed h-full"
			:class="!isFlod ? 'w-[260px]' : 'w-0 md:w-[260px]'"
			data-projection-id="130"
		>
			<!-- 展开按钮 -->
			<div
				class="md:hidden absolute -right-16 top-4 border border-gray-300 rounded-md hover:bg-gray-50 w-8 h-8 flex justify-center items-center"
				@click="isFlod = false"
				v-if="isFlod"
			>
			...
			<!-- nav -->
			<div
				class="p-4 flex overflow-hidden flex-col whitespace-nowrap bg-slate-50 h-full"
				:class="!isFlod ? 'w-[260px]' : 'w-0 px-0 md:p-4 md:w-[260px]'"
			>
				<!-- 标题 -->
				<div class="flex justify-between relative">
					<h1 class="text-xl"><span>&#128295;</span><span class="ml-2 font-serif">GPT TOOLS</span></h1>
					<!-- 收起按钮 -->
					<div
						class="md:hidden border border-gray-300 rounded-md hover:bg-gray-50 duration-100 w-8 h-8 flex justify-center items-center"
						@click="isFlod = true"
					>
						<span class="-rotate-90">
							<i class="iconfont icon-fold text-gray-500 text-xs"></i>
				...
				<!-- 创建聊天 -->
				<button
					class="flex items-center justify-center w-full mt-4 p-2 border- text-sm gray border bg-white rounded-md hover:bg-gray-50"
					@click="createChat"
				>
				...
				<button
					...
					class="mt-4 w-[32px] h-[32px] bg-white hover:bg-gray-100 rounded-lg flex justify-center items-center shadow"
				>
